...
export class MenuItemsService {
    ...
    */
    ...
    async getMenuItems() {
        const parseChildrens = (items: MenuItem[], id: Number | null = null): MenuItem[] => {
            const current = items
                .filter((item: MenuItem) => item.parentId === id)
            const newMenuItem = current.map((item: MenuItem) => ({
                ...item,
                children: parseChildrens(items, item.id),
            }));

            return newMenuItem;
        }
        const menuItems = await this.menuItemRepository.find();
        return parseChildrens(menuItems);
    }
}